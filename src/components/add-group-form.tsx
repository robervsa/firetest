...
export default function AddGroupForm({ onGroupAdded }: AddGroupFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!user) {
        toast({ title: 'Error', description: 'Debes iniciar sesión para agregar un grupo.', variant: 'destructive' });
        return;
    }
    ...
    if (error) {
        toast({
            title: 'Error',
            description: `Hubo un error al agregar el grupo: ${error.message}`,
            variant: 'destructive'
        })
    } else {
        ...
        toast({
          title: 'Grupo Añadido',
          description: `El grupo "${values.name}" ha sido agregado.`,
        });
        form.reset();
    }
  }
